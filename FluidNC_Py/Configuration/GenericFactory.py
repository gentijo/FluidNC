...
class BuilderBase():
    ...
    def __init__(self):
        self.name_:str = None
        self.builders_:list[BuilderBase] = []
        self.objects_:list[BaseType] = []
    ...
    def create(const char* name) -> BaseType:
        pass
...
class InstanceBuilder(BuilderBase):
    ...
    def factory(Configuration::HandlerBase& handler, BaseType*& inst) ->None:
        if (inst == nullptr) {
    ...
    #// "using" declaration.
    def factory(Configuration::HandlerBase& handler) {
        objects = instance().objects_;
