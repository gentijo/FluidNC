...
class ParserHandler(HandlerBase):
        Configuration::Parser&   _parser;
        std::vector<const char*> _path;

    void enterSection(const char* name, Configuration::Configurable* section):
        ...
        if (thisIndent > entryIndent) {
            ...
            for (; _parser._token._indent >= thisIndent; _parser.Tokenize()):
                log_parser_verbose(" KEY " << _parser.key() << " state " << int(_parser._token._state) << " indent " << _parser._token._indent);
    ...
    void item(const char* name, Macro& value):
        if (_parser.is(name)) {
